...
class ArticlesBootstrap(
    ...
    fun generateDummyArticles(): List<Article> {
        ...
        val article2 = Article(date = LocalDate.now(ZoneOffset.UTC).minusDays(10),
            ...
            content = "b Content data of article 2. Article 2 explores the issues regarding the problems without solutions on current days"
        )
        articles.add(article2)

        val article3 = Article(date = LocalDate.now(ZoneOffset.UTC).minusDays(2),
            title = "Article 3: article title",
            content = "c Content data of article 3. Article 3 explores the issues regarding the problems without solutions on current days"
        )
        ...
        val article4 = Article(date = LocalDate.now(ZoneOffset.UTC),
            ...
            content = "d Content data of article 4. Article 4 explores the issues regarding the problems without solutions on current days"
        )
